...
create view vWConfidentialData
as
select * from UserDetails where CreatedDate>='2021-12-02 10:11'

select * from  vWConfidentialData

create view vWSummarizeddata
 as
SELECT Designation,city, sum(salary) as totalSalary, count(id) as totalemployee
FROM UserDetails
...
select * from  vWbygroupname
 ...
 create view vWbygroupnamewithoutnull
 as
select userDetails.FirstName, Grouptable.Id,grouptable.Names from  UserDetails  join UserGroupTable on   UserDetails.ID =UserGroupTable.UserId 
...
select * from   vWbygroupnamewithoutnull
